...
class RunnerBase:
    ...
    def use_fsdp(self):
        if self.use_distributed:
            return self.config.run_cfg.get('fsdp',False)
        else:
            assert False, "when fsdp set to True, distributed must be true"
    ...
    def grad_norm_clip(self):
        return self.config.run_cfg.get("grad_norm_clip",None)
